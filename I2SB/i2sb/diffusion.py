...
class Diffusion():
    ...
    def q_sample(self, step, x0, x1, ot_ode=False, detach=True):
    ...
    def p_posterior(self, nprev, n, x_n, x0, ot_ode=False):
        ...
        # Handle both scalar and batched tensors
        if mu_x0.ndim == 0:
            mu_x0 = mu_x0[None].expand(x0.shape[0])
            mu_xn = mu_xn[None].expand(x0.shape[0]) 
            var = var[None].expand(x0.shape[0])
            nprev = torch.tensor(nprev, device=self.device)[None].expand(x0.shape[0])

        xt_prev = mu_x0[:, None, None, None] * x0 + mu_xn[:, None, None, None] * x_n
        
        if not ot_ode:
            noise_mask = (nprev > 0).float()[:, None, None, None]
            xt_prev = xt_prev + noise_mask * var.sqrt()[:, None, None, None] * torch.randn_like(xt_prev)

        return xt_prev

    def ddpm_sampling(self, steps, pred_x0_fn, x1, mask=None, ot_ode=False, log_steps=None, verbose=True, train_mode=False):
        ...
        for prev_step, step in pair_steps:
            ...
            if prev_step in log_steps and not train_mode:
