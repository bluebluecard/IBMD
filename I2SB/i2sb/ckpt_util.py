...
def build_ckpt_option(opt, log, ckpt_path):
    ...
    overwrite_keys = ["use_fp16", "device", "model_add_noise_input"]
    ...
    if hasattr(opt, "checkpoint_name"):
        ckpt_opt.checkpoint_name = opt.checkpoint_name

    if hasattr(opt, "model_name_in_ckpt"):
        ckpt_opt.model_name_in_ckpt = opt.model_name_in_ckpt

    if hasattr(opt, "model_ema_name_in_ckpt"):
        ckpt_opt.model_ema_name_in_ckpt = opt.model_ema_name_in_ckpt

    if hasattr(opt, "distillation_checkpoint"):
        ckpt_opt.distillation_checkpoint = opt.distillation_checkpoint
        debug()
        if hasattr(opt, "distillation_checkpoint_name"):
