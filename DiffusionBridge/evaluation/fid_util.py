...
FID_REF_DIR = Path("assets/stats")


def collect_features(
    dataset, mode, batch_size, num_workers, device=torch.device("cuda"), use_dataparallel=True, verbose=True
):
...
def compute_fid_ref_stat(opt, log):
    from datasets.imagenet_inpaint import build_lmdb_dataset
    ...
    lmdb_dataset = build_lmdb_dataset(opt.dataset_dir, opt.image_size, train=opt.split == "train", transform=transform)
    ...
    mu, sigma = collect_features(dataset, mode, batch_size=512, num_workers=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--split", type=str, choices=["train", "val"], help="which dataset to compute FID ref statistics"
    )
