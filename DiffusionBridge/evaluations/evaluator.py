...
def get_cmmd(args):
    model = ClipEmbeddingModel(torch.device("cuda"), use_dataparallel=True)
    ...
    if "pool_3" in obj:
        ...
        if not ("mu" in obj and "sigma" in obj and "act" in obj):
            with open_npz_array(args.ref_batch, "arr_0") as reader:
                ref_acts = np.asarray(model.compute_activations(reader.read_batches(512)))
        else:
            ref_acts = obj["act"]

    with open_npz_array(args.sample_batch, "arr_0") as reader:
        sample_acts = np.asarray(model.compute_activations(reader.read_batches(512)))

    print("Computing evaluations...")
    cmmd = mmd(ref_acts, sample_acts).item()
    metrics = {}
    ...
    save_dir = os.path.dirname(os.path.realpath(args.sample_batch))
    with open(os.path.join(save_dir, "cmmd.json"), "w") as f:
        json.dump(metrics, f)
        
        
def get_cmmd_saved_cmmd(args):
    model = ClipEmbeddingModel(torch.device("cuda"), use_dataparallel=True)
    ...
    if "pool_3" in obj:
        ...
        if not ("mu" in obj and "sigma" in obj and "act" in obj):
            ...
            found_cmmd_activation = False
        else:
            ref_acts = obj["act"]
            found_cmmd_activation = True
...
def get_kid(args):
    ...
    if "pool_3" in obj:
        ...
        if not ("mu" in obj and "sigma" in obj and "act" in obj):
            with open_npz_array(args.ref_batch, "arr_0") as reader:
                kid_activations(reader.read_batches(512), kid, True)
        else:
            ref_acts = obj["act"]

    with open_npz_array(args.sample_batch, "arr_0") as reader:
        kid_activations(reader.read_batches(512), kid, False)
    ...
    save_dir = os.path.dirname(os.path.realpath(args.sample_batch))
    with open(os.path.join(save_dir, "kid.json"), "w") as f:
        json.dump(metrics, f)
...
def _resize_bicubic(images, size):
    ...
    images = images.permute(0, 2, 3, 1).numpy() / 255.0
    return images

class ClipEmbeddingModel:
    """CLIP image embedding calculator."""

    def __init__(self, device, use_dataparallel):
    ...
    def compute_activations(self, batches):
        ...
        return all_embs.astype("float32")

def kid_activations(batches, kid, is_real):
    for batch in tqdm(batches):
        th_batch = torch.from_numpy(batch)
        th_batch = torch.permute(th_batch, (0, 3, 1, 2))
        kid.update(th_batch, real=is_real)
