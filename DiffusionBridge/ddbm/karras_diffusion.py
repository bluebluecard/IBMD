...
class KarrasDenoiser:
    ...
    def training_bridge_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if mask is not None:
            denoised = mask * denoised + (1. - mask) *  xT
        ...
        return terms, x_t, denoised
    
    def training_student_losses(self, model, bridge_model, x_start, t, model_kwargs=None, noise=None):
        assert model_kwargs is not None
        xT = model_kwargs["xT"]
        mask = model_kwargs.pop("mask", None)
        if noise is None:
        ...
        _, denoised_teacher, weights = self.denoise(model, x_t, t,  **model_kwargs)
        _, denoised_bridge, weights = self.denoise(bridge_model, x_t, t,  **model_kwargs)

        if mask is not None:
            denoised_bridge = mask * denoised_bridge + (1. - mask) *  xT
            denoised_teacher = mask * denoised_teacher + (1. - mask) *  xT
            true_loss = mean_flat(weights * mask * (denoised_teacher - x_start) ** 2)
            fake_loss = mean_flat(weights * mask * (denoised_bridge - x_start) ** 2)
            terms["mse"] = true_loss - fake_loss
        else:
            true_loss = mean_flat(weights * (denoised_teacher - x_start) ** 2)
            fake_loss = mean_flat(weights * (denoised_bridge - x_start) ** 2)
            terms["mse"] = true_loss - fake_loss

        terms["loss"] = terms["mse"]

        return terms, x_t, denoised_teacher, denoised_bridge
