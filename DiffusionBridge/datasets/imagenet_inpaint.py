...
def _my_build_lmdb_dataset(root, transform=None, target_transform=None, loader=lmdb_loader):
    """
    You can create this dataloader using:
    train_data = _build_lmdb_dataset(traindir, transform=train_transform)
    valid_data = _build_lmdb_dataset(validdir, transform=val_transform)
    ...
    path_to_save = "/mnt/ar_hdd/ sel/imagenet_lmdb"
...
def build_lmdb_dataset_val10k_lmdb_folder_path(dataset_dir, image_size, transform=None, 
                                               lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
    ...
    if transform is None: transform = build_test_transform(image_size)
    # dataset = _build_lmdb_dataset(opt.dataset_dir / 'val', log, transform=transform)
    
    dataset_val_dir = os.path.join(dataset_dir, "val")
    
    dataset = _build_lmdb_dataset_lmdb_folder_path_val10k(dataset_val_dir, transform=transform,
    ...
    print(f"[Dataset] Built Imagenet val10k, size={len(dataset)}!")
    return dataset


def _my_build_lmdb_dataset_lmdb_folder_path(root, transform=None, target_transform=None, loader=lmdb_loader,
                                            lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
    """
    You can create this dataloader using:
    train_data = _build_lmdb_dataset(traindir, transform=train_transform)
...
def _nirvana_my_build_lmdb_dataset(root, transform=None, target_transform=None, loader=lmdb_loader,
                                         lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
    """
    You can create this dataloader using:
    train_data = _build_lmdb_dataset(traindir, transform=train_transform)
...
def my_build_lmdb_dataset(dataset_dir, image_size, train, transform=None):
    """resize -> crop -> to_tensor -> norm(-1,1)"""
    fn = os.path.join(dataset_dir, "train" if train else "val")
    ...
    dataset = _my_build_lmdb_dataset(fn, transform=transform)
    # log.info(f"[Dataset] Built Imagenet dataset {fn=}, size={len(dataset)}!")
    return dataset

def my_build_lmdb_dataset_lmdb_folder_path(dataset_dir, image_size, train, transform=None,
                                           lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
    """resize -> crop -> to_tensor -> norm(-1,1)"""
    fn = os.path.join(dataset_dir, "train" if train else "val")
    ...
    dataset = _my_build_lmdb_dataset_lmdb_folder_path(fn, transform=transform,
                                                      lmdb_folder_path=lmdb_folder_path)
    # log.info(f"[Dataset] Built Imagenet dataset {fn=}, size={len(dataset)}!")
    return dataset

def nirvana_build_lmdb_dataset_lmdb_folder_path(dataset_dir, image_size, train, transform=None, 
                                                lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
    """ resize -> crop -> to_tensor -> norm(-1,1) """
    fn = os.path.join(dataset_dir, "train" if train else "val")
    ...
    dataset = _nirvana_my_build_lmdb_dataset(fn, transform=transform,
                                             lmdb_folder_path=lmdb_folder_path)
    print(f"[Dataset] Built Imagenet dataset {fn=}, size={len(dataset)}!")
    return dataset


def _my_build_lmdb_dataset_val10k(dataset_dir, image_size, train, transform=None):
    """resize -> crop -> to_tensor -> norm(-1,1)"""
    fn = dataset_dir
...
def _my_build_lmdb_dataset_val10k_lmdb_folder_path(dataset_dir, image_size, train, transform=None,
                                                   lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
    """resize -> crop -> to_tensor -> norm(-1,1)"""
    fn = dataset_dir
...
def nirvana_my_build_lmdb_dataset_val10k(dataset_dir, image_size, transform=None):
    fn = os.path.join(dataset_dir, "val")
    fn_10k = readlines("assets/datasets/val_faster_imagefolder_10k_fn.txt")
    label_10k = readlines("assets/datasets/val_faster_imagefolder_10k_label.txt")
...
def my_build_lmdb_dataset_val10k(dataset_dir, image_size, transform=None):
    fn = os.path.join(dataset_dir, "val")
    fn_10k = readlines("assets/datasets/val_faster_imagefolder_10k_fn.txt")
    label_10k = readlines("assets/datasets/val_faster_imagefolder_10k_label.txt")
    ...
    if transform is None:
        transform = build_test_transform(image_size)
    dataset = _my_build_lmdb_dataset_val10k(fn, image_size, False, transform=transform)
    dataset.samples = [(os.path.join(fn, name), int(label)) for name, label in zip(fn_10k, label_10k)]
...
def my_build_lmdb_dataset_val10k_lmdb_folder_path(dataset_dir, image_size, transform=None,
                                                  lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
    fn = os.path.join(dataset_dir, "val")
    fn_10k = readlines("assets/datasets/val_faster_imagefolder_10k_fn.txt")
    label_10k = readlines("assets/datasets/val_faster_imagefolder_10k_label.txt")
    ...
    if transform is None:
        transform = build_test_transform(image_size)
    dataset = _my_build_lmdb_dataset_val10k_lmdb_folder_path(fn, image_size, False, transform=transform,
                                                             lmdb_folder_path=lmdb_folder_path)
    dataset.samples = [(os.path.join(fn, name), int(label)) for name, label in zip(fn_10k, label_10k)]
...
class MyInpaintingVal10kSubset(Dataset):
    def __init__(self, dataset_dir, image_size, corrupt_type, transform=None):
        super(MyInpaintingVal10kSubset, self).__init__()
        self.dataset = my_build_lmdb_dataset_val10k(dataset_dir, image_size, transform)
...
class MyInpaintingVal10kSubsetLMDBFolder(Dataset):
    def __init__(self, dataset_dir, image_size, corrupt_type, transform=None,
                 lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
        super(MyInpaintingVal10kSubsetLMDBFolder, self).__init__()
        self.dataset = my_build_lmdb_dataset_val10k_lmdb_folder_path(dataset_dir, image_size, 
                                                                     transform, lmdb_folder_path=lmdb_folder_path)
...
class NirvanaMyInpaintingVal10kSubset(Dataset):
    def __init__(self, dataset_dir, image_size, corrupt_type, 
                 lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
        super(NirvanaMyInpaintingVal10kSubset, self).__init__()
        # self.dataset = nirvana_my_build_lmdb_dataset_val10k(dataset_dir, image_size, transform)
        self.dataset = build_lmdb_dataset_val10k_lmdb_folder_path(dataset_dir, image_size, 
                                                                  lmdb_folder_path=lmdb_folder_path)
...
class MyImageNetInpaintingDataset(Dataset):
    def __init__(self, dataset_dir, image_size, corrupt_type, train, transform=None):
        super(MyImageNetInpaintingDataset, self).__init__()
        self.dataset = my_build_lmdb_dataset(dataset_dir, image_size, train, transform)
...
class MyImageNetInpaintingDatasetLMDBFolder(Dataset):
    def __init__(self, dataset_dir, image_size, corrupt_type, train, transform=None,
                 lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
        super(MyImageNetInpaintingDatasetLMDBFolder, self).__init__()
        self.dataset = my_build_lmdb_dataset_lmdb_folder_path(dataset_dir, image_size, train, transform,
                                                              lmdb_folder_path=lmdb_folder_path)
...
class NirvanaMyImageNetInpaintingDataset(Dataset):
    def __init__(self, dataset_dir, image_size, corrupt_type, train, transform=None,
                 lmdb_folder_path="/mnt/ar_hdd/ sel/imagenet_lmdb"):
        super(NirvanaMyImageNetInpaintingDataset, self).__init__()
        self.dataset = nirvana_build_lmdb_dataset_lmdb_folder_path(dataset_dir, image_size, train, transform,
                                                                   lmdb_folder_path=lmdb_folder_path)
