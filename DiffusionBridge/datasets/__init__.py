...
def load_data_inpainting(
    ...
    corrupt_type = dataset.split("_")[-1]
    print(f"corrupt type = {corrupt_type}")
    assert corrupt_type in ["center", "freeform2030"]
    from .imagenet_inpaint import MyImageNetInpaintingDataset, MyInpaintingVal10kSubset

    trainset = MyImageNetInpaintingDataset(root, image_size, corrupt_type, train=True)
    testset = MyInpaintingVal10kSubset(root, image_size, corrupt_type)
...
def load_data_inpainting_lmdb_folder(
    data_dir,
    dataset,
    batch_size,
    image_size,
    ...
    print(f"corrupt type = {corrupt_type}")
    assert corrupt_type in ["center", "freeform2030"]
    from .imagenet_inpaint import MyImageNetInpaintingDatasetLMDBFolder, MyInpaintingVal10kSubsetLMDBFolder

    trainset = MyImageNetInpaintingDatasetLMDBFolder(root, image_size, corrupt_type, train=True,
                                                     lmdb_folder_path=lmdb_folder_path)
    testset = MyInpaintingVal10kSubsetLMDBFolder(root, image_size, corrupt_type, lmdb_folder_path=lmdb_folder_path)
...
def load_data_inpainting_nirvana(
    data_dir,
    dataset,
    batch_size,
    image_size,
    ...
    print(f"corrupt type = {corrupt_type}")
    assert corrupt_type in ["center", "freeform2030"]
    from .imagenet_inpaint import NirvanaMyImageNetInpaintingDataset, NirvanaMyInpaintingVal10kSubset

    trainset = NirvanaMyImageNetInpaintingDataset(root, image_size, corrupt_type, train=True,
                                                  lmdb_folder_path=lmdb_folder_path)
    print(f"finished to prepare train set!")
    testset = NirvanaMyInpaintingVal10kSubset(root, image_size, corrupt_type, lmdb_folder_path)
    print(f"finished to prepare val set!")
